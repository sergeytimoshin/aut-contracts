...
describe("PluginRegistry", (accounts) => {
  before(async function () {
    [deployer, admin, verifier, addr1, addr2, addr3, ...addrs] =
  ...
  describe("Plugin Registry", async () => {
    it("Adds a standalone plugin type definition", async () => {
      let tx = pluginRegistry.addPluginDefinition(
        addr1.address,
        "ipfs://abcdef123456789",
        ethers.utils.parseEther("0.3"),
        true,
        []
      );
      await expect(tx)
        .to.emit(pluginRegistry, "PluginDefinitionAdded")
        .withArgs(1);
    });
    ...
    it("Fails if can't be standalone but paid", async () => {
      ...
      ).to.be.revertedWith("AUT: Should be free if not standalone");
    });

    it("Checks the plugin definition", async () => {
      let pluginTypeDefinition = await pluginRegistry.pluginDefinitionsById(1);
      const dependencies = await pluginRegistry.getDependencyModulesForPlugin(
        1
      );
      expect(pluginTypeDefinition.creator).to.equal(addr1.address);
      expect(pluginTypeDefinition.metadataURI).to.equal(
        "ipfs://abcdef123456789"
      );
      expect(pluginTypeDefinition.price).to.equal(
        ethers.utils.parseEther("0.3")
      );
    ...
    it("Changes price", async () => {
      await pluginRegistry
        .connect(addr1)
        .setPrice(1, ethers.utils.parseEther("0.2"));
    });
    ...
    it("Fails to change price from wrong account", async () => {
      await expect(
        pluginRegistry
          .connect(addr2)
          .setPrice(1, ethers.utils.parseEther("0.2"))
      ).to.be.revertedWith("AUT: Only creator can set price");
    });
    it("Should deploy a standalone plugin", async () => {
      ...
      expect(offchainVerifiedTaskPlugin.address).not.to.eq(
        ethers.constants.AddressZero
      );
    });

    it("Should add a free plugin to the DAO", async () => {
      let tx = pluginRegistry
        .connect(admin)
        .addPluginToDAO(offchainVerifiedTaskPlugin.address, 2);
      await expect(tx)
        .to.emit(pluginRegistry, "PluginAddedToDAO")
        .withArgs(1, 2, nova.address);
    });
    ...
    it("Should fail if payment is incorrect", async () => {
      await expect(
        pluginRegistry
          .connect(admin)
          .addPluginToDAO(offchainVerifiedTaskPlugin.address, 1, {
            value: ethers.utils.parseEther("0.15"),
          })
      ).to.be.revertedWith("AUT: Insufficient price paid");
    });
